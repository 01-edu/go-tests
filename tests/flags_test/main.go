...
func main() {
	...
	lib.Program("flags", node.flagsShorthand[0]+"v2", "v1")
	lib.Program("flags", node.flagsShorthand[1], "v1")
	lib.Program("flags", "-h")
	lib.Program("flags", "--help")
	lib.Program("flags")

	for _, v2 := range node.randArgFlag {
		for _, v1 := range node.randArg {
			lib.Program("flags", node.flags[0]+v2, node.flags[1], v1)
		}
	}
	for _, v2 := range node.randArgFlag {
		for _, v1 := range node.randArg {
			lib.Program("flags", node.flagsShorthand[0]+v2, node.flagsShorthand[1], v1)
		}
	}
}
